...
// returns true if successful, false otherwise
{
	...
	// read the file
	cout << "Reading the read counts..." << endl;
	_CALL(
		read_tsv( ifs, genes, num_genes, arrays, num_arrays, this_EOL )
		);

	cout << *num_genes << " genes were read, each with a profile size of "
		<< *num_arrays << "." << endl << endl;
...
// returns true if successful, false otherwise
{
	...
	_CALL(
		read_metadata( ifs, arrays, num_arrays, groups, num_groups, this_EOL )
		);

	cout << *num_groups << " groups were read." << endl << endl;
